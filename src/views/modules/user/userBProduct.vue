					...
					>
						<template slot-scope="{row}">
							<span style="margin-left: 10px">{{
									getPaidList(row.paidList)[index]?getPaidList(row.paidList)[index].value:'-' }}</span>
							<el-button text v-if="getPaidList(row.paidList)[index] && getPaidList(row.paidList)[index].status===0">确认缴费</el-button>
						</template>
					</el-table-column>
	...
	methods: {
		...
		submitForm (formName) {
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					const totalAmount = this.userForm.amount * this.detailItem.totalPhases
					this.$http({
